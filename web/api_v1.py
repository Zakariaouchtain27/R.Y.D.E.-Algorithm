...
def _charge_success_fee_sync(
    ...
    Always called synchronously from inside the PRISM executor thread —
    never blocks the async event loop.

    Writes one of three audit events per booking:
      billing_charged  — Stripe PaymentIntent succeeded
      billing_error    — Stripe charge failed; ryde.billing_error webhook fired
      billing_skipped  — no Stripe customer ID on file for this agency
    """
...
def _run_prism_sync(
    ...
    # Charge 20% success fee whenever PRISM fires a STRIKE decision.
    # In Signal API mode the agency acts on the webhook; we bill at decision
    # time since there is no separate rebooking-confirmation back-channel.
...
def _run_predict_sync(
    ...
    # Free the (N_PATHS \xd7 horizon) ndarray immediately to keep RAM flat.
    del paths
    gc.collect()
