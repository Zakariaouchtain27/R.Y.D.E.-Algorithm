...
class CompetitiveCascadeDetector:
    ...
    _LN_MU = 1.5      # log-normal mean for cascade delay (hours)
...
class LoadFactorPressureModel:
    ...
    TOTAL_SEATS = 180   # typical narrowbody
    ...
    def pressure(
        ...
        if observed_velocity is not None and observed_velocity > 0:
            ...
            # Heuristic: extrapolate from observed pace assuming 90-day window
        ...
        if shortfall <= 0:
            return 0.05  # on track to fill — minimal pricing pressure

        # Sigmoid centered at 30% shortfall, steep slope
