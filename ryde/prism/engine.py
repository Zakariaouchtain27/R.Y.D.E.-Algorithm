...
Decision framework:
    ...
    Overrides:
      Load factor pressure >78%  → downgrade STRIKE → PHANTOM_HOLD
        (airline likely to cut again — wait for a lower floor)
      Cascade probability >72%   → upgrade WAIT → PHANTOM_HOLD
        (competitor drop expected to cascade — lock current fare as safety net)
"""
...
class PRISMEngine:
    ...
    def evaluate(
        ...
        historical_max_drop: Optional[float] = None,   # kept for API compatibility
        ...
        # Fit OU parameters from recorded price history
        ...
        # Core ratio: how much of the provable expected value are we capturing now?
        ratio = net_savings / expected_value if expected_value > 0 else 1.0

        # Load factor pressure (leading indicator: will airline cut again?)
        ...
        # Competitive cascade probability
        ...
        # Override 1: High load factor pressure → airline will cut more → wait
        ...
        # Override 2: Competitor cascade signal → lock current fare as safety net
