"""
Agency and API key management — SQLite-backed.

Keys are stored in plaintext for MVP simplicity.
In production: store HMAC-SHA256(key) and compare hashes.
"""
import re
import secrets
import sqlite3
...
class AgencyStore:
    ...
    def _seed_dev_keys(self) -> None:
        ...
        for agency_id, name, email, key in seeds:
            self._conn.execute(
                """
                INSERT OR IGNORE INTO agencies (id, name, email, api_key, environment)
                VALUES (?, ?, ?, ?, 'test')
                """,
                (agency_id, name, email, key),
            )
        self._conn.commit()
    ...
    def revoke(self, agency_id: str) -> None:
        with self._lock:
            self._conn.execute(
                "UPDATE agencies SET active = 0 WHERE id = ?", (agency_id,)
            )
            self._conn.commit()
    ...
    def reactivate(self, agency_id: str) -> None:
        with self._lock:
            self._conn.execute(
                "UPDATE agencies SET active = 1 WHERE id = ?", (agency_id,)
            )
            self._conn.commit()
    ...
    @staticmethod
    def _from_row(row: sqlite3.Row) -> Agency:
        return Agency(
            id=row["id"],
            name=row["name"],
            email=row["email"],
